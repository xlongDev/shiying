...
describe("GET /api/stream (SSRF 防护)", () => {
  ...
  it("内网 / 元数据 / localhost 地址一律被拒绝 (403)", async () => {
    ...
    for (const target of targets) {
      const res = await GET(makeReq(target, `stream-ssrf-${Math.random()}`));
